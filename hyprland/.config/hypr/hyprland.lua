...
local browser2     = "waterfox"
local notepad      = "nvim-qt"
...
hl.on("hyprland.start", function () 
  ...
  hl.exec_cmd("swaybg -i ~/.config/hypr/background/red-future-3136x1792.jpg")
  hl.exec_cmd("waybar")
  hl.exec_cmd("fcitx5")
...
-- to hopefully make Dolphin open files with programs correctly
...
hl.config({
    general = {
        ...
        col = {
            active_border   = "rgba(ff5555aa)",
         -- active_border   = { colors = {"rgba(33ccffee)", "rgba(00ff99ee)"}, angle = 45 }, --default
            inactive_border = "rgba(595959aa)",
         -- inactive_border = "rgba(595959aa)", --default
        },
...
hl.bind(mainMod .. " + SHIFT + E", hl.dsp.exec_cmd(fileManager2))
...
-- Move focus with mainMod + arrow keys (vim bindings)
...
-- Switch workspaces with mainMod + SHIFT + arrow keys (vim bindings)
...
-- # Other
-- # lock
-- # bind = $mainMod, U, exec, swaylock -f -c 000000
...
hl.bind("Print", hl.dsp.exec_cmd([[if area=$(slurp); then grim -g "$area" - | tee >(wl-copy) > ~/Pictures/screenshots/Screenshot-$(date +%F_%T).png && notify-send "Screenshot of the region taken" -t 1000; fi]]))
hl.bind("SHIFT + Print", hl.dsp.exec_cmd([[grim ~/Pictures/screenshots/Screenshot-$(date +%F_%T).png && notify-send "Screenshot of the whole screen taken" -t 1000]]))
...
---- my window rules
-- make pavucontrol (for audio) pop up in floating window by default
hl.window_rule({
...
-- make xdg-desktop-portal-termfilechooser pop up in floating window by default
-- HEY you need to fix this at some point because it still doesn't work
hl.window_rule({
